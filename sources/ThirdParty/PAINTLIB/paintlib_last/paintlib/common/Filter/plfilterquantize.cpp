...
/--------------------------------------------------------------------
|
|      $Id: plfilterquantize.cpp,v 1.5 2001/10/21 17:12:40 uzadow Exp $
|
...
void PLFilterQuantize::SetUserPalette(const PLPixel32* pPal)
{
  ...
  memcpy(m_pUserPal, pPal, sizeof(PLPixel32)*256);
}
...
void PLFilterQuantize::genMedianPalette (PLBmp * pBmpSource, PLBmp * pBmpDest) const
{
  ...
  for( i=0; i<256; i++ )
  {
    ...
    PLBYTE r = ((long)(m_pQuBoxes[i].Corner0.GetR ()+m_pQuBoxes[i].Corner1.GetR ()))*255/(2*31);
    PLBYTE g = ((long)(m_pQuBoxes[i].Corner0.GetG ()+m_pQuBoxes[i].Corner1.GetG ()))*255/(2*31);
    PLBYTE b = ((long)(m_pQuBoxes[i].Corner0.GetB ()+m_pQuBoxes[i].Corner1.GetB ()))*255/(2*31);
...
{
  PLPixel32 * pPal = pBmpDest->GetPalette();
  memcpy (pPal, DefaultPalette, 256*4);
}
...
void PLFilterQuantize::ditherDestBmp(PLBmp * pBmpSource, PLBmp * pBmpDest) const
{
  ...
  if (m_DitherType == PLDTH_FS)
  {
    ...
    memset(pOddErrors, 0, (w + 2) * 6 * sizeof(double));
  }
