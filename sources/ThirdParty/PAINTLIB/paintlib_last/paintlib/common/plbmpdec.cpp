...
/--------------------------------------------------------------------
|
|      $Id: plbmpdec.cpp,v 1.7 2001/10/21 17:12:39 uzadow Exp $
    ...
    )
{
  ...
  {
    PLPixel32 Pal[256];
    pBMI = getInfoHeader (pDataSrc, &Pal[0]);

    int DestBPP = pBMI->biBitCount;
    bool bAlpha = false;
    if (DestBPP == 32)
      bAlpha = true;
    if (DestBPP > 8)
    ...
    pBmp->Create (pBMI->biWidth, pBMI->biHeight, DestBPP, bAlpha);
    if (DestBPP == 8)
      pBmp->SetPalette (&Pal[0]);
    ...
    try
    {
      ...
      {
        case 1:
          decode1bpp (pDataSrc, pBmp);
          break;
        case 4:
          if (pBMI->biCompression == BI_RGB)
            decode4bpp (pDataSrc, pBmp);
           else
            decodeRLE4 (pDataSrc, pBmp);
          break;
        case 8:
          if (pBMI->biCompression == BI_RGB)
            decode8bpp (pDataSrc, pBmp);
           else
            decodeRLE8 (pDataSrc, pBmp);
          break;
        case 16:
          decodeHiColor (pDataSrc, pBmp, pBMI);
          break;
        case 24:
        case 32:
          decodeTrueColor (pDataSrc, pBmp, pBMI->biBitCount);
          break;
        default:
	    ...
	    if (pBMI->biXPelsPerMeter > 0)
		    DPI.x = (int)((float)pBMI->biXPelsPerMeter / 39.37f+0.5);
	    if (DPI.x <= 1)
		    DPI.x = 150;

	    if (pBMI->biYPelsPerMeter > 0)
		    DPI.y = (int)((float)pBMI->biYPelsPerMeter / 39.37f+0.5);
	    if (DPI.y <= 1)
		    DPI.y = 150;
    ...
    )
{
  ...
  else
  {
    ...
    // Colors are really bit masks.
    PLULONG * pColMask;
    pDataSrc->Read4Bytes();  // Skip blue bitmask
    pColMask = (PLULONG *) pDataSrc->ReadNBytes(4);
    pDataSrc->Read4Bytes();  // Skip red bitmask
    if (*pColMask == 0x03E0)
      decodeTrueColor (pDataSrc, pBmp, 15);
    else
    ...
    )
{
  ...
  memcpy (pDest, pSrc, pBmp->GetWidth()*4);
}
